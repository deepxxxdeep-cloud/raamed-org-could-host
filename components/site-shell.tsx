  ...
  return <header className="sticky top-0 z-40 overflow-visible border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
    <div className="mx-auto flex max-w-7xl items-center gap-6 px-5 py-3 lg:px-8">
      <Link href="/" className="group flex min-w-0 shrink-0 items-center gap-3" aria-label="Raamed home">
        <span className="flex h-14 w-44 shrink-0 items-center overflow-hidden rounded-2xl bg-white p-1 shadow-sm ring-1 ring-slate-200 transition group-hover:-translate-y-0.5 group-hover:shadow-md sm:h-16 sm:w-52"><img src={logoUrl} alt="Raamed logo" className="h-full w-full object-contain" /></span>
      </Link>
      <nav className="hidden min-w-0 flex-1 items-center justify-center gap-7 text-[13px] font-semibold text-slate-600 lg:flex"><div className="group relative"><button className="flex items-center gap-1 whitespace-nowrap transition hover:text-[#f36f2b]">Products <ChevronDown className="size-3.5 transition group-hover:rotate-180" /></button><div className="invisible absolute left-1/2 top-full z-50 w-64 -translate-x-1/2 translate-y-2 rounded-2xl border border-slate-200 bg-white p-2 opacity-0 shadow-xl transition duration-200 group-hover:visible group-hover:translate-y-1 group-hover:opacity-100">{productLinks.map(([label, href]) => <Link key={href} href={href} className="block rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-orange-50 hover:text-[#f36f2b]">{label}</Link>)}<Link href="/products" className="mt-1 block rounded-xl border-t border-slate-100 px-3 py-2.5 text-sm font-bold text-[#0c6670]">View all equipment</Link></div></div>{links.map(([label, href]) => <Link key={href} href={href} className="whitespace-nowrap transition hover:text-[#f36f2b]">{label}</Link>)}</nav>
      <div className="hidden shrink-0 items-center gap-3 lg:flex"><a href="tel:+919625970722" className="flex items-center gap-2 text-sm font-semibold text-[#102a43]"><Phone className="size-4 text-[#f36f2b]" />+91 96259 70722</a><button onClick={onQuote} className="whitespace-nowrap rounded-full bg-[#f36f2b] px-4 py-3 text-sm font-bold text-white shadow-lg shadow-orange-500/15 transition hover:-translate-y-0.5 hover:bg-[#dd5b1d]">Request a quote</button></div>
      <button className="rounded-xl border border-slate-200 bg-white p-2 text-[#102a43] shadow-sm lg:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X /> : <Menu />}</button>
    </div>
    {open && <nav className="flex flex-col gap-1 border-t border-slate-200 bg-white px-5 py-4 lg:hidden"><Link href="/products" onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 font-bold text-[#0c6670] hover:bg-orange-50">Products</Link>{productLinks.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className="rounded-xl px-3 py-2 pl-6 text-sm font-semibold text-slate-600 hover:bg-orange-50 hover:text-[#f36f2b]">{label}</Link>)}{links.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 font-semibold text-slate-700 hover:bg-orange-50 hover:text-[#f36f2b]">{label}</Link>)}<Link href="/#quote" onClick={() => setOpen(false)} className="mt-2 rounded-xl bg-[#f36f2b] px-4 py-3 text-center font-bold text-white">Request a quote</Link></nav>}
  </header>
}

export function SiteFooter() {
  return <footer className="bg-[#102a43] px-5 py-14 text-white lg:px-8"><div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[1.3fr_1fr_1fr]"><div><img src={logoUrl} alt="Raamed logo" className="h-16 w-56 rounded-xl bg-white object-contain p-1" /><p className="mt-5 max-w-sm text-sm leading-7 text-slate-300">Reliable medical equipment, practical expertise, and responsive support for the teams who keep care moving.</p></div><div><p className="font-bold text-orange-300">Explore</p><div className="mt-4 flex flex-col gap-3 text-sm text-slate-300"><Link href="/about">About us</Link><Link href="/offices">Regional offices</Link><Link href="/customers">Customers</Link><Link href="/products">Products</Link><Link href="/media">Media centre</Link></div></div><div><p className="font-bold text-orange-300">Talk to our team</p><div className="mt-4 flex flex-col gap-3 text-sm text-slate-300"><a href="tel:+919625970722">+91 96259 70722</a><a href="mailto:Business@raamed.org">Business@raamed.org</a><p>Mon–Sat, 9:00–18:00 IST</p></div></div></div><div className="mx-auto mt-12 max-w-7xl border-t border-white/15 pt-5 text-xs text-slate-400"><span>© 2026 Raamed. Built for dependable care.</span><span className="mt-2 block text-[11px] text-slate-500">Made by <a href="https://aiwebify.site" target="_blank" rel="noreferrer" className="underline underline-offset-2 transition hover:text-orange-300">aiwebify.site</a></span></div></footer>
}
